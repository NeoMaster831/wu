...
def test_result(mat, _target_pubkey, curve):
...
def recover_private_key(
    signatures_data, h_int, _pub_key, curve, bits_type, num_bits, loop
):
    ...
    while loop_var:
        ...
        for effort in RECOVERY_SEQUENCE:
            ...
            cand1, cand2 = test_result(lattice, None, curve)
            return cand1, cand2
        loop_var = loop
        if loop:
            print("One more try")
...
def lattice_attack_cli(file_name, loop):
    ...
    cand1, cand2 = recover_private_key(
        signatures, hash_int, None, curve_string, data_type, known_bits, loop
    )
    if cand1 and cand2:
        print("Key found \\o/")
        print(hex(cand1), hex(cand2))
    else:
