...
# This file was *autogenerated* from the file akali.sage
from sage.all_cmdline import *   # import sage library

_sage_const_0xFF = Integer(0xFF); _sage_const_2 = Integer(2); _sage_const_8 = Integer(8); _sage_const_4 = Integer(4); _sage_const_3 = Integer(3); _sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_16 = Integer(16); _sage_const_15 = Integer(15); _sage_const_23 = Integer(23)
cl = bytes.fromhex("""
...
def gf_mul(a: int, b: int):
    return exp_list[(log_list[a] + log_list[b]) % _sage_const_0xFF ]

def gf_pow(a: int, b: int):
    return exp_list[(b * log_list[a]) % _sage_const_0xFF ]

F = GF(_sage_const_2 )['x']  # 유한체 GF(2)에서 다항식 변수 'x' 설정
p = x**_sage_const_8  + x**_sage_const_4  + x**_sage_const_3  + x**_sage_const_2  + _sage_const_1   # 생성 다항식 설정
FF = GF(_sage_const_2 **_sage_const_8 , modulus=p, names=('a',)); (a,) = FF._first_ngens(1)# 유한체 GF(2^8) 생성

for i in range(_sage_const_0 , len(cl), _sage_const_16 ):
    
    cd = cl[i:i+_sage_const_16 ]
    m = []
    v = []
    for j in range(_sage_const_16 ):
        ...
        for k in range(_sage_const_16 ):
            s = gf_add(s, gf_mul(gf_pow(gf_pow(_sage_const_2 , j), k), cd[_sage_const_15  - k]))
        v.append(-FF.fetch_int(s))
        
        Row = []
        for k in range(_sage_const_8 ):
            Row.append(FF.fetch_int(gf_pow(gf_pow(_sage_const_2 , j), _sage_const_23  - k)))
        m.append(Row)
